...
unsigned int Surface::CompOutCode( float x, float y )
{
	unsigned int code = 0;
	if (y > m_YMax) code |= TOP; else if (y < m_YMin) code |= BOTTOM;
	if (x > m_XMax) code |= RIGHT; else if (x < m_XMin) code |= LEFT;
	return code;
}
bool Surface::Clip( float& x0, float& y0, float& x1, float& y1 )
{
	bool accept = 0, done = 0;
	unsigned int outcode0 = CompOutCode( x0, y0 ), outcodeOut;
	unsigned int outcode1 = CompOutCode( x1, y1 );
	do
	{
		if (!(outcode0 | outcode1)) accept = done = true; else if (outcode0 & outcode1) done = true; else 
		{
			float x, y;
			outcodeOut = outcode0 ? outcode0 : outcode1;
			if (outcodeOut & TOP) x = x0 + (x1 - x0) * (m_YMax - y0) / (y1 - y0), y = m_YMax;
			else if (outcodeOut & BOTTOM) x = x0 + (x1 - x0) * (m_YMin - y0) / (y1 - y0), y = m_YMin;
			else if (outcodeOut & RIGHT) y = y0 + (y1 - y0) * (m_XMax - x0) / (x1 - x0), x = m_XMax;
			else y = y0 + (y1 - y0) * (m_XMin - x0) / (x1 - x0), x = m_XMin;
...
void Surface::Line( int x1, int y1, int x2, int y2, Pixel color )
{
	Line( (float)x1, (float)y1, (float)x2, (float)y2, color );
}
...
void Surface::Line( float x1, float y1, float x2, float y2, Pixel c )
{
	...
	for ( int i = 0; i <= il; i++ ) *(m_Buffer + (int)x1 + (int)y1 * m_Pitch) = c, x1 += dx, y1 += dy;
}

void Surface::Plot( int x, int y, Pixel c )
{ 
	if ((x >= m_XMin) && (y >= m_YMin) && (x < m_XMax) && (y < m_YMax)) m_Buffer[x + y * m_Pitch] = c;
}
