...
public:
	...
	void ClipTo( float x1, float y1, float x2, float y2 ) { m_XMin = x1, m_XMax = x2, m_YMin = y1, m_YMax = y2; }
	// Special operations
	void InitCharset();
	...
	unsigned int CompOutCode( float x, float y );
	bool Clip( float& x0, float& y0, float& x1, float& y1 );
